...
# real-world fallback: permissions are limited to pushing these ECR
# repos only (no EC2, no IAM, no billing access), but it is a standing
# credential rather than OIDC's short-lived token exchange.
#
# Scoped to 4 repos now that the frontend is also deployed to the
# cluster, not just run locally.
