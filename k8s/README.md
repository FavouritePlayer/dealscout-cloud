...
1. Substitutes `${ECR_API_IMAGE}` / `${ECR_SCRAPER_IMAGE}` / `${ECR_QDRANT_IMAGE}` / `${ECR_FRONTEND_IMAGE}` with the actual ECR repo URLs from Terraform output.
...
3. Applies these manifests in order: `qdrant.yaml`, `listings-cache-pvc.yaml`, `api.yaml`, `scraper.yaml`, `frontend.yaml`.
